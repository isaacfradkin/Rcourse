...
מזל שלמדנו לולאות. בשיעורים הקודמים למדנו שכל חזרה על לולאה נקראית איטרציה, כדי לבצע מבחן בוטסטרפ צטרך לבצע מספר רב של איטרציות אבל נתחיל מלהוסיף מעט חזרות, כדי שנוכל ממש לראות את התוצר של כל חזרה נתחיל עם 5 חזרות.  
...
הרצנו את הלולאה ועכשיו יכולנו לראות 5 מדגמים שונים כמובן ש5 ערכים הם עדיין לא התפלגות ממש. לכן נצטרך להוסיף עוד חזרות כדי לדמות את תפלגות הממוצע. לשם כך אנחנו צריכים לשמור את הממוצעים של המדגמים המדומים בוקטור.  
...
במקרה זה הסיבה שנבחר בחישוב הא-פרמטרי שלרווח הסמך היא שנרצה להשתמש בחציון ולא בממוצע.  
...
# נציג
paste0("the bootastaped CI is: ", round(lower), " - ", round(upper), " ILS")
...
print(paste0("The difference between original distribution and bootsrtap mean is: ", mean(bootstrap_data)-mean(original_mean)))
...
קיבלנו תוצאה קצת מדהימה! ממדגם אחד שדגם 30 משתתפים מתוך האוכלוסיה כולה הצלחנו לשחזר כמט במדוייק את התפלגות הדגימה של האוכלוסיה, כפי שחולצה מתוך האוכלוסיה. זאת למרות שאנחנו יודעים שהאוכלוסיה לא מתפלגת נורמלי. בתיבה מטה נמצאת גרסה מתומצתת של הקוד. עכשיו אתם יכולים להתנסות בעצמכם.  
...
נוכל להשתמש במבחן זה במקרה בו הנחות הסטטיסטיקה הפרמטרית מופרות (וגם אם לא), רק שבמקום להשוות את סטטיסטי להתפלגות יידועה (למשל התפלגות t) אנחנו נייצר התפלגות המייצגת את השערת האפס מנתוני המדגם הנוכחי.  
...
בגדול היא מניחה שהחלוקה לקבוצות היא אקראית וחסרת משמעות. בעצם אם המשתנה הבלתי תלוי חסר השפעה זה לא משנה מאיזה קבוצה נדגום, אנחנו נקבל אומדן זהה לחציון.  
...
עכשיו שהבנו את העיקרון של חישוב סטטיסטי על סמך חלוקה אקראית לקבוצות נוכל לעבור לחלק של ביצוע לולאת הפרמוטציות. העיקרון מאוד דומה לבוטסטרפ. בואו נראה:
...
אפשר להסיק מכך שההסתברות לקבל כזאת תוצאה היא מאוד לא סבירה תחת השערת האפס.

`תרגיל`  
היחידה למחקר רצתה להבין האם יכול להיות שיש פער מגדרי בנתוני השכר.
חזרו על המבחן שביצענו והשוו את הפרשי החציונים בין המגדרים להתפלגות האפס והדפיסו את ערך הp שהתקבל .  
בדקו את ההפרש בין גברים לנשים (כלומר החסירו את משכורות הנשים משל הגברים)  
רמז - האם צריך לשנות משהו בלולאה עצמה? 

```{r perm_2samp_ex, exercise=TRUE, exercise.eval = FALSE }
set.seed(234)
n_iter <- 10000
group_size <- nrow(salary_df)/2
H0_dist <- numeric(n_iter)
...
אפשר ללמוד מהתרגיל שני דברים חשובים:
עם שינוי מאוד קטן בקוד יכולנו להשתמש באותה לולאה כדי לדמות את התפלגות האפס של שני מבחנים שונים. לכן האופן בו נשתמש בלולאות יכול לשמש אותנו במקרים רבים. 
כדי לעבור בין השערה חיובית לשלילית אפשר פשוט לשנות את האופן שבו אנחנו מחסירים את החציונים זה מזה.
...
למשל אם נתרגם את הדוגמה המקורית שלנו למבחן מזווג, נוכל לבחון את ההשכר של כל משתתף במדגם לפני ואחרי לימודי התכנות. לכל משתתף בעצם יש שתי תצפיות שתלויות זו בזו. את זהות הנבדקים אנחנו לא נרצה לערבב, שכן גם תחת השערת האפס לא כוללת בתוכה את ההנחה שאין קשר בין תצפיות של אותו הנבדק אלא רק שאין משמעות לסדר - מה הייתה המדידה שלפני ומה הייתה המדידה שאחרי המניפולציה.
...
question("איך אתם מציעים להתאים את התהליך למבחן זה?",
         ...
         answer("עכשיו בכל פרמוטציה נרצה לדגום במקרה חצי מהנבדקים ולהפוך את סדר התצפיות שלהם (השכר שהרוויחו לפני לימודי התכנות יוחלף עם השכר שהרוויחו אחרי)", correct = TRUE, message = " "),
...
# נסתכל על ההבדל
print("Original Before Vector: ")
print(before)
print("Shuffled Before Vector: ")
print(temp_before)

# חלק מהתצפיות הוחלפו בתצפית שאחרי המניפולציה
# נתבונן בהשפעה על ההפרשים
print("Original Difference Vector: ")
print(after-before)
print("Shuffled Difference Vector: ")
print(temp_after-temp_before)

# ואיך זה השפיע על ממוצע ההפרשים?
print(paste0("Original Difference Mean: ", mean(after-before)))
print(paste0("Shuffled Difference Mean: ", mean(temp_after-temp_before)))
...
ביצוע של המבחן באופן הזה משנה מעט את ביצוע שלו ואת הניסוח של השערת האפס באופן קונקרטי, אך לא מהותית. השערת האפס היא עדיין שאין חשיבות לאיזה תצפית הייתה לפני המניפולציה ואיזה אחריה ולכן תחת השערת האפס הכיוון של ההפרש (האם מחסירים את הערך שלפני המניפולציה מהערך שאחריה) לא משנה.  
אם נחשוב על זה מהזוית של ההפרשים אפשר לנסח את ההשערה גם כך - השערת האפס היא שתוחלת ההפרשים היא אפס. כלומר אם נסתכל על הוקטור של ההפרשים הם אמורים להתפזר אקראית סביב אפס. כלומר, הסימן (האם הערכים הם חיוביים או שליליים) של ההפרשים הוא אקראי.  
אם נחשוב על זה כך, נוכל לבצע את המבחן אחרת, ללא הלולאה הפנימית. 
נשתמש בוקטור האמיתי של ההפרשים ובנוסף אליו נדגום אקראית וקטור בעל אורך זהה שמכיל -1 ו1. נכפול את הוקטורים, מה שיגרום לכך שהסימן של חלק מההפרשים יתהפך. 
...
עכשיו אנחנו יכולים לבחון שאלות כמו זאת: במפעל מסוים רצו לבדוק האם המטבעות המיוצרים במפעל הינם מטבעות הוגנים. כלומר, האם הסיכוי לקבל "עץ" שווה לסיכוי לקבל "פלי". במפעל דגמו 30 מטבעות מפס היצור והטילו כל אחד מהם 10 פעמים וחישבו את הסיכוי לקבל "עץ" על פני החזרות. עכשיו אפשר להשוות את המדגם לסיכוי של מטבע הוגן - 0.5.  
ההתאמה היחידה שנצטרך לבצע היא בעצם למצוא את הדרך להפוך את ערך הייחוס ל0 שוב. נוכל לעשות זאת על ידי החסרה של ערך הייחוס מהנתונים עצמם.

`תרגיל`  
כתבו את הקוד שיבצע מבחן פרמוטציות ויבדוק האם המטבע הוגן. הנתונים קיימים כבר בסביבת העבודה שלכם בוקטור שנקרא: coin_sample. כל ערך בוקטור מייצג את הסיכוי לקבל עץ על פני 10 חזרות לכל מטבע שנדגם.  


```{r one_samp_perm_ex, exercise=TRUE, exercise.eval = FALSE }
set.seed(156)
# הנתונים  
print(coin_sample)
...
grade_result(pass_if(~identical(.result, 0.4)),
    ...
    correct="מצויין! קיבלנו סיכוי  של 0.3 לקבל מדגם כזה תחת השערת האפס. כלומר לא נוכל לדחות את השערת האפס שהמטבעות הוגנים.", 
...
question("מהו נכון לגבי רווח הסמך?",
         answer("הווח הסמך כולל את הסיכוי לקבל תשובה נכונה במקרה (0.5)",  message ="לא נכון, בדקו את הקוד שלכם. "),
         ...
         answer("קיבלנו שכולל בתוכו גם את הסיכוי לקבל תשובה נכונה במקרה (0.5) וגם את הערך 1.",  message = " לא נכון, קיבלנו ערך צר מאוד והערך המתואר כאן רחב."),
         allow_retry = TRUE
)
```

`שאלה 2`  
אחד החוקרים חשד שיש אפקט סדר במדגם, כך שככל שמספר הנבדק גבוה יותר כך הדיוק גבוה יותר. בחנו את ההשערה באמצעות מבחן פרמוטציות.


```{r final_q2, echo = FALSE}

question_text(
  "בכמה פרמוטציות התקבל ערך קורלציה גבוה מהערך שהתקבל במדגם?",
  answer("1760", correct = TRUE),
  answer("0.17", correct = TRUE),
  answer("0.18", correct = TRUE),
  answer("0.176", correct = TRUE),
  allow_retry = TRUE,
...
`שאלה 3`   
ועכשיו לאירוע המרכזי, חזרו לטבלה המקורית והוסיפו עמודה שמגדירה האם הצעד היה תואם או לא תואם והאם. סננו תשובות שגויות ותשובות שזמן התגובה שלהן מעל 1500 מילי שניות.  
בצעו מבחן פרמוטציות לדגימות תלויות על אפקט הסטרופ.  

```{r final_q3_1, echo = FALSE}

question_text(
  "מה הממוצע של התפלגות האפס?",
  answer("0.01185977", correct = TRUE),
  answer("0.012", correct = TRUE),
  answer("0.011", correct = TRUE),
  answer("0.0118", correct = TRUE),
  allow_retry = TRUE,
...
question("דרג/י את רמת הקושי של החומר המועבר ביחידה לדעתך",type="single",
         answer("(1) קל מאוד",  correct=TRUE, message ="!שמחים לשמוע שהיה לך קל"),
         answer("(2) קל",  correct=TRUE, message = "!שמחים לשמוע שהיה לך קל"),
         answer("(3) בינוני",  correct=TRUE, message = "מקווים שלמרות האתגר את/ה מרגיש/ה שלמדת"),
         answer("(4) מאתגר",  correct=TRUE ,message = "מקווים שלמרות האתגר את/ה מרגיש/ה שלמדת"),
         answer("(5) מאתגר מאוד",  correct=TRUE,  message = "מקווים שלמרות האתגר את/ה מרגיש/ה שלמדת"),
         correct = "",
         incorrect = "",
         allow_retry = TRUE
)
...
question("עד כמה הלומדה העבירה את החומר בצורה ברורה?",type="single",
         answer("(1) ברור מאוד",  correct=TRUE,message ="!שמחים לשמוע"),
         answer("(2) ברור",  correct=TRUE,  message ="!שמחים לשמוע"),
         answer("(3) בינוני",  correct=TRUE, message = "תודה על המשוב הכנה והמועיל. נשמח אם תפרט/י למטה"),
         answer("(4) לא ברור",  correct=TRUE,  message = "תודה על המשוב הכנה והמועיל. נשמח אם תפרט/י למטה"),
         answer("(5) לא הבנתי כלום",  correct=TRUE,  message = "תודה על המשוב הכנה והמועיל. נשמח אם תפרט/י למטה"),
         correct = "",
         incorrect = "",
         allow_retry = TRUE
)
...
question("אילו נושאים בלומדה היו קשים במיוחד?",type="learnr_checkbox",
