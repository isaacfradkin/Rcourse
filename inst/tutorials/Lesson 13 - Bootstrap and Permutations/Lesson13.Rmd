...
temp_programing <- all_salary_temp[1:groupP_size]
temp_no_programing <- all_salary_temp[(groupP_size+1):nrow(salary_df)]
...
for (i in 1:n_iter){
  ...
  all_salary_temp <- sample(all_salary_data, replace = FALSE)
  # החצי הראשון יהיה הקבוצה שלמדה תכנות
  temp_programing <- all_salary_temp[1:groupF_size]
  # והחצי השני יהיה קבוצה שלא
  temp_no_programing <- all_salary_temp[(groupF_size+1):nrow(salary_df)]
  # נחשב את הפרש החציונים
  temp_median_diff <- median(temp_programing) - median(temp_no_programing)
  #נאכסן את התוצאה
  H0_dist[i] <- temp_median_diff
...
```{r one_samp_perm_ex, exercise=TRUE, exercise.eval = FALSE,exercise.reveal_solution = FALSE}
