...
##### פונקציה לבדיקה
exercises_df = data.frame(exercises = c("t_dist_q", "critical_t_ex", "one_samp_t_output_question", "two_sample_t_q", "paired_t_q", "outlier_t_ex", "final_q_5", "final_q_6", "final_q_7", "final_q_8_1", "final_q_8_2", "final_q_9", "final_q_10", "final_q_11"), hebrew = c("שאלה - התפלגות ", "שאלה - ערך קריטי", "שאלה - מבחן למדגם בודד", "שאלה - מבחן שאלה - מדגמים בלתי תלויים", "תרגיל - מבחן למדגמים מזווגים", "תרגיל - תצפיות קיצוניות", "תרגיל מסכם שאלה 1", "תרגיל מסכם שאלה 2", "תרגיל מסכם שאלה 3", "תרגיל מסכם שאלה 4", "תרגיל מסכם שאלה 5", "תרגיל מסכם שאל 6", "תרגיל מסכם שאלה 7", "תרגיל מסכם שאלה 8"
                                     ))
...
נוכל לחשב את הערך הקריטי באמצעות הפונקציה `qt()` אשר, בדומה לqnorm(), מחשבת מהו הערך המתכתב עם אחוז מסוים של ההתפלגות ודרגות החופש שלה. 
לפונקציה זאת לא נציין את התוחלת וסטיית התקן שכן אלו מקובעות בפונקציה לתוחלת של 0 וסטיית תקן של 1.
...
נוכל לעשות זאת על ידי שימוש בפונקציה ההפוכה לqt() - `pt()`. אליה נוכל להכניס את הערך שחושב במדגם ולקבל מה ההסתברות שלו תחת התפלגות הt. או במילים אחרות לחשב את ה`p-value`. 
...
לא ברור לכם איזה ערך קיבל הp value במבחן הנוכחי? תקבלו הסבר מלא בחלק הבא.
 ...
 כן, מתוארת לנו גם ההשערה האלטרנטיבית עבור המבחן שערכנו – שהתוחלת של האוכלוסייה ממנה דגמנו אינה 2.  
 מתיאור זה נוכל להסיק מה היא השערת האפס, שהתוחלת שווה ל2 כמו כן, תיאור זה מבטא שהמבחן שערכנו היה דו זנבי.  
...
מערך הtהקיצוני וגם מערך ה p ניתן להסיק שקיבלנו תוצאה מובהקת. אבל יש כמה נקודות  שכדאי לשים לב אליהן בפלט.   
...
למדגם בודד על הפרשי זמני התגובה. כיוון שיש לנו השערה לגבי כיווניות האפקט הסטרופ השתמשו בהנחה המתאימה ושמרו את ערך הtהמחושב במשתנה.
...
question("איך הייתם פותרים את הבעיה הזאת?",
         answer("ניתן להחליף את הערך הקיצוני בערך חסר", message ="זאת אכן אפשרות נכונה, אך היא לא היחידה. במקרה זה כיוון שמדובר במבחן מזווג אנחנו המבחן יתעלם מהערך החסר וגם מהערך המצומד אליו."),
         answer("ניתן להסיר את התצפית מהנתונים וגם את התצפית המזווגת אליה ",message = "הסרה של התצפית הקיצונית והתצפית המזווגת היא אכן אפשרות נכונה אך היא לא היחידה"),
         answer("ניתן להחליף את הערך הקיצוני בממוצע המדגם", message = " זאת גם אפשרות בחלק מהמקרים, הייתרון שלה הוא שהיא מאפשרת לנו לשמר את דרגות החופש וממוצע המדגם הוא הערכה סבירה של הערך. אבל במקרה זה, בו ערך חריג בתנאי אחד מעלה את הנבדק כולו בספק, עדיף לבחור באופציה שמסירה אותו מהניתוח."),
...
נחזור לדוגמה שהתחלנו איתה, החוקר המקומי החליט שכדאי שהוא ישחזר את הממצאים   דוברי עברית שלו ודגם דגימה נוספת של משתתפים מקומיים. אם המדגם הראשון היה אמין ואמד בצורה מדוייקת את זמן התגובה של המשתתפים לא נצפה שתהיה תוצאה שונה באופן חריג במדגם השני. הנתונים טעונים ללומדה בתוך טבלה שנקראית outlier_data.
...
## תרגיל מסכם
ברור לנו שלאורך כל הלומדה רק חיכיתם לעבוד עם נתונים אמיתיים והנה הגיע הרגע. בהם השתמשתם לתרגיל המסכם בסמסטר הקודם על מטלת הסטרופ.  
...
הציגו גרף קופסא אשר מציג את הפרשי הממוצעים.
