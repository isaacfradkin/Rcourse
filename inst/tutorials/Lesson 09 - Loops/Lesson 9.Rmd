...
נוכל להיעזר בשיטה זו גם כדי לשמור תוצרים שחושביו מתוך הנתונים שלנו. למשל:
...
התוצאה זהה, והשיטה המומלצת היא דווקא זו של השמה לפי אינדקס.   
הראנו לכם את שיטה זו בעיקר כי היא עוזרת לנו להבין איך לפעול כשנרצה לשמור תוצאות לטבלה.

במקרים בהם אנחנו רוצים לשמור מספר תוצאות בכל סבב  של הלולאה אנחנו יכולים לייצר מספר וקטורים ריקים שונים, ולשמור בכל אחד תוצאה אחרת (למשל וקטור נפרד לממוצעים, סטיות תקן, ערך מינמום וכו'), אבל לפעמים יהיה יותר יעיל לשמור את התוצאות יחד בטבלה - שאליה נוסיף את התוצאות שלנו בכל סבב של הלולאה.
...
# כתבו כאן את הלולאה
# נרצה לעבור על הערכים בין 1 ל5
for (__________){
  
  # שם הנבדק שנחשב עבורו את המתאם בסבב זה
  subj = 
  
  #  שליפת הנתונים של הנבדק מתוך הטבלה 
  # social_sleep
  social_sleep_subj = 
  
  # חישוב המתאם בין העמודות
  # sleep ו social
  r =  
...
`source("name_of_file.R")`. נתרגל את השימוש בשיטה זו בסוף היחידה.
...
הוסיפו בחלונית זו שורת קוד שתייבא את קובץ הRDS שייצרתם באמצעות הפקודה `readRDS`.שמרו את הטבלה המיובאת בשם "long_data".
...
עיינו בקובץ הדיווח שנוצר וענו על השאלה הבאה:
