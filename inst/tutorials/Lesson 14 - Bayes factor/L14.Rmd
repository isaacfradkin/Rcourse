...
question(
 ...
 מה המסקנה מניתוח זה?
  ...
  answer("ממוצע שעות השינה הינו 7 שעות"),
  answer("לא ניתן לקבוע כי ממוצע שעות השינה שונה מ7 שעות"),
  answer("ממוצע שעות השינה שונה מ7 שעות", correct = T),
  allow_retry = TRUE,
...
בדוגמה שלנו, שנוגעת לשעות השינה של סטודנטים, השערת האפס היא שתוחלת שעות השינהה שלהם היא 7 שעות, או במונחים של גודל אפקט: שערך הCohen's d עבור ההפרש של התוחלת מ7 הוא 0.
...
המאפיינים של ההתפלגות גורמים לכך שחצי מההתפלגות תמיד תהיה מוגבלת בין r ל-r ולכן המשמעות של קביעה הערך של r ב1, למשל, היא שאנחנו משערים שיש 50% שגודל האפקט הוא בין 1 ל-1. 

הערך שמהווה את ברירת המחדל עבור r ברוב הניתוחים הוא $\sqrt2/2 = 0.707$, שכן ערך זה מהווה השערה שאינה מניחה גדלי אפקט גדולים או קטנים מדי.
...
נהוג להשתמש ברף של פי 3, כך שאם הBF שקיבלנו גדול מערך ה נוכל לקבל את ההשערה האלטרנטיבית ואם לא, למשל אם קיבלנו BF של 1.5, נוכל להגיד שההשערה האלטרנטיבית אמנם נראית טיפה יותר סבירה בהינתן הנתונים שלנו, אבל התוצאות לא חזקות מספיק כדי שנוכל להסיק מהן מסקנות חד משמעיות.

כדי להבין יותר לעמוק את התוצאות נוכל להמיר אותן ל*הסתברויות* לכך שכל אחת מההשערות נכונה באמצעות הנוסחה הבאה:
...
כך שאם ייחסנו לשתי ההשערות הסתברות זהה להיות נכונות לפני שאספנו את הנתונים שלנו, ההסבתרות ש$H_1$ תהיה נכונה בהינתן הנתונים תהיה:
...
BF_to_prob = function(BF, prior_H1){
  ...
  p_H1_given_D = (BF * prior_H1) / (BF * prior_H1 + prior_H0)
  p_H0_given_D = 1 - p_H1_given_D
...
posteriors = BF_to_prob(BF = 1.5, prior_H1 = 0.5)

ggplot(posteriors, aes(x = H))+
  geom_col(aes(y = posterior, fill = H))+
  geom_col(aes(y = prior), fill = NA, col = "black", linetype = 2)+
  ylim(c(0,1))+
  labs(x = "Hypothesis", y = "Probability", title = "Prior and posterior probabilities", fill = "Posterior")+
  theme_minimal()

```

היעזרו בקוד הנ"ל כדי לענות על השאלה הבאה:
...
question_text(
  "<div style='direction: rtl;'>
  בהינתן אמונה מקדימה המייחסת הסתברות זהה לשתי ההשערות וBF לטובת H1 השווה ל4, מה ההסתברות המעודכנת שH1 נכון לכך ?
  </div>",
...
פעולה זו לא משנה את התוכן של הניתוח - אבל מציגה אותו בצורה ההפוכה: פי כמה השערת האפס סבירה יותר מההשערה האלטרנטיבית.
...
בכל ניתוח בייסיאני שנריץ, נתחיל בלבחירת ערך סקאלה המתאים להשערות שלנו לגבי האפקטים שאנחנו מצפים למצוא. נוכל להשתמש בערך ברירת המחדל (0.707), אבל רצוי להתאים את הערך לציפיות שלנו בכל ניתוח.
...
bf1 = ttestBF(x = data, mu = 0, r = 0.707)
bf2 = ttestBF(x = data, mu = 0, r = 0.5)
bf3 = ttestBF(x = data, mu = 0, r = 0.3)

bf = c(bf1,bf2,bf3)@bayesFactor$bf

df = bind_rows(df,
               as.data.frame(t(c(ES,mn,bf))))
}

names(df) = c("ES","mn","a","b","c")
...
ההבדל היחיד הוא שבמבחן חד זנבי עבור מתאמים נצטרך לעדכן את הטווח של הביטוי `nullInterval=c(0,Inf)` כך שבמקום הערך אינסוף (`Inf`) יופיע הערך 1, שכן במקרה זה גודל האפקט אינו יכול להיות גדול מ1.


## תרגיל


ברשותכם טבלה בשם "attention_task" נתוני מהירות תגובה ממוצעת ואחוזי דיוק במטלת קשב עבור 50 נבדקים.
