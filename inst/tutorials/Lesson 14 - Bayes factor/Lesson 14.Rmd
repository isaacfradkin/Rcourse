...
הערך שמהווה את ברירת המחדל עבור r ברוב הניתוחים הוא $\sqrt2/2 = 0.707$, שכן ערך זה מניח גדלי אפקט בינוניים.
...
BF_to_prob = function(BF, prior_H1){
  ...
  p_H1_given_D = (BF * prior_H1) / (BF * prior_H1 + prior_H0) # עדכון הסיכוי לטובת ההשערה האלטרנטיבית 
...
# יצירת תרשים המציג את התוצאות
ggplot(posteriors, aes(x = H))+
  geom_col(aes(y = posterior, fill = H))+
  geom_col(aes(y = prior), fill = NA, col = "black", linetype = 2)+
  ylim(c(0,1))+
  labs(x = "Hypothesis", y = "Probability", title = "Prior (dashed lines) and posterior probabilities", fill = "Posterior")+
  theme_minimal()
...
`תרגיל`
הניחו כי אחרי שעדכנו את ההסתברויות בעד H1 וH0 אספנו מדגם נוסף. הפעם ערך הBF שקיבלנו הוא 2.67.

חשבו מה הסיכוי שH1 נכונה **אחרי** שלקחנו בחשבון את התוצאות מ**2 המדגמים**.
 
לשם כך תצטרכו לחשב את הסיכוי לכך שH1 נכון על סמך תוצאות המדגם הראשון, לעדכן את האמונה המקדימה בהתאם לתוצאות ולעדכן אותה שוב על סמך התוצאות מהמדגם השני.

התוצאות מהעדכון הראשון ישמשו אותנו כאמונה המקדימה עבור העדכון השני.
...
question_text(
  "<div style='direction: rtl;'>
  בהינתן אמונה מקדימה המייחסת הסתברות זהה לשתי ההשערות וBF לטובת H1 השווה ל4, מה ההסתברות המעודכנת שH1 נכונה ?
  
  הזינו את התשובה כמסםר בין 0 ל1 ועגלו ל2 מיקומים אחרי הנקודה העשרונית
  </div>",
...
השימוש בפונקציות האחרות שכלולות בחבילה הוא די דומה, אבל עם שינוי קל שנוגע להשערה האלטרנטיבית. בעוד גודל האפקט של מבחן t יכול להיות כל מספר שהוא, גודל האפקט עבור מתאם מוגבל בין הערכים 1 ו 1-.
